...
import Registration from '@/server/events/datathon/model';
import { validateRegistration } from '@/server/events/datathon/validation';
...
const serverError = (context, error) => {
  console.error(`[datathon/registrations] ${context}:`, error);
...
export async function POST(req) {
  try {
    ...
    const kaggleEmails = members.map((m) => m.kaggleEmail.toLowerCase());
    const universityIds = members.map((m) => m.universityId.toLowerCase());

    // 2. Duplicate Checks in current request
    const duplicateEmails = kaggleEmails.filter(
      (e, i) => kaggleEmails.indexOf(e) !== i
    );
    ...
    const duplicateUnivIds = universityIds.filter(
      (s, i) => universityIds.indexOf(s) !== i
    );
    ...
    const existingTx = await Registration.findOne({ transactionId: transactionId.trim() });
    if (existingTx) {
      return NextResponse.json(
        {
          success: false,
    ...
    const existingEmails = await Registration.findOne({
      'members.kaggleEmail': { $in: kaggleEmails },
    });
    if (existingEmails) {
      return NextResponse.json(
        {
    ...
    const created = await Registration.create({
      ...
      registrationId,
      paid: false, // Default to unpaid until admin approves
    });
