...
import AppChallengeRegistrationForm from '@/components/AppChallengeRegistrationForm';
...
export const metadata = {
  title: 'App Challenge Registration — PSTU IT Carnival 2026',
  description:
    'Register for App Challenge at PSTU IT Carnival 2026. Free Registration sponsored by BDAPPS.',
};
  ...
  <div className="mx-auto max-w-2xl px-4 py-16 sm:py-24">
    <div className="rounded-2xl border border-ink-600 bg-ink-800/60 p-6 shadow-card sm:p-8">
      <div className="flex items-start gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-magenta-400/10 text-magenta-300">
        ...
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-mist-300">
            The registration form will open shortly. Check back soon!
          </p>
        </div>
      </div>
        ...
        <ul className="mt-3 space-y-2.5">
          {event.registration.checklist.map((item) => (
            <li key={item} className="flex items-start gap-3">
              <CheckIcon className="mt-0.5 h-4 w-4 shrink-0 text-magenta-400" />
...
export default function AppChallengeRegisterPage() {
