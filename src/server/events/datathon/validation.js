...
export function validateRegistration(body) {
  ...
  // Members validation
  if (!members || !Array.isArray(members) || members.length < 1 || members.length > 2) {
    ...
    members.forEach((m, idx) => {
      ...
      if (!m.universityName || typeof m.universityName !== 'string' || m.universityName.trim().length === 0) {
        errors.push({ field: `members[${idx}].universityName`, message: 'University name is required' });
      }
      if (!m.universityId || typeof m.universityId !== 'string' || m.universityId.trim().length === 0 || m.universityId.trim().length > 50) {
        errors.push({ field: `members[${idx}].universityId`, message: 'University ID is required' });
      }
      ...
      if (!m.kaggleEmail || typeof m.kaggleEmail !== 'string' || !EMAIL_RE.test(m.kaggleEmail.trim())) {
        errors.push({ field: `members[${idx}].kaggleEmail`, message: 'Please provide a valid Kaggle email' });
      }
      if (!m.kaggleUsername || typeof m.kaggleUsername !== 'string' || m.kaggleUsername.trim().length === 0) {
        errors.push({ field: `members[${idx}].kaggleUsername`, message: 'Kaggle username is required' });
      }
    });
  }
